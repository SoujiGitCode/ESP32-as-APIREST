...
void setup() {
  Serial.begin(115200);                               // setting up a
  ...
  server.on("/", []() {                               // define here wat the webserver needs to do
    server.send(200, "text/html", jsonWeb);           //    -> it needs to send out the HTML string "webpage" to the client
  });
...
void loop() {
  ...
  if ((unsigned long)(now - previousMillis) > interval) { // check if "interval" ms has passed since last time the clients were updated
    ...
    object["a1"] = random(100);                    // write data into the JSON object -> I used "rand1" and "rand2" here, but you can use anything else
    object["a2"] = random(100);
    object["a3"] = random(100);                    // write data into the JSON object -> I used "rand1" and "rand2" here, but you can use anything else
